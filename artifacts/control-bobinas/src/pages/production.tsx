...
function OrderCard({ order, index, onDelete, onEdit, onBlock, onUnblock, blocked = false, actionPending = false }: { order: ProductionOrder; index: number; onDelete?: () => void; onEdit?: () => void; onBlock?: () => void; onUnblock?: () => void; blocked?: boolean; actionPending?: boolean }) {
  ...
  return <article className={`load-in relative rounded-xl border bg-card p-5 sm:p-6 ${blocked ? 'border-accent/50' : 'border-border'}`} style={{ animationDelay: `${index * 55}ms` }} data-testid={`card-order-${order.id}`}>
    {blocked ? <button type="button" onClick={onUnblock} disabled={actionPending} className="pressable absolute right-5 top-5 flex min-h-11 items-center gap-2 rounded-lg border border-primary/25 px-3 text-xs font-semibold text-primary hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50 sm:right-6 sm:top-6" data-testid={`button-unblock-order-${order.id}`}><Unlock size={16} /> Desbloquear</button> : <button type="button" onClick={onBlock} disabled={actionPending} className="pressable absolute right-5 top-5 flex min-h-11 items-center gap-2 rounded-lg border border-accent/60 bg-secondary/70 px-3 text-xs font-semibold text-accent-foreground hover:bg-secondary disabled:cursor-not-allowed disabled:opacity-50 sm:right-6 sm:top-6" data-testid={`button-block-order-${order.id}`}><Lock size={16} /> Bloquear</button>}
    <div className="flex flex-col gap-5 xl:flex-row xl:items-center">
      <div className="min-w-0 flex-1 pr-28"><div className="flex items-center gap-3"><span className="rounded-md bg-primary px-2.5 py-1 font-data text-[11px] font-semibold text-primary-foreground" data-testid={`text-order-id-${order.id}`}>ORD-{String(order.id).padStart(4, '0')}</span><span className={`flex items-center gap-1.5 font-data text-[10px] uppercase tracking-wider ${statusClass}`}><span className={`h-1.5 w-1.5 rounded-full ${statusDotClass}`} /> {order.estado}</span></div><h2 className="mt-4 font-display text-[2.35rem] font-semibold leading-none">{order.ancho} <span className="text-xl font-medium text-muted-foreground">mm</span><span className="mx-2 text-muted-foreground/40">/</span>{order.micras} <span className="text-xl font-medium text-muted-foreground">µ</span></h2><p className="mt-2 text-sm text-muted-foreground">Camisa <strong className="text-foreground">{order.camisa}</strong> <span className="mx-1.5 text-muted-foreground/40">·</span> {order.material}</p></div>
      ...
      {!blocked && <><button type="button" onClick={onEdit} className="pressable flex min-h-11 items-center justify-center gap-2 rounded-lg border border-primary/25 px-3 text-xs font-semibold text-primary hover:bg-muted" data-testid={`button-edit-order-${order.id}`}>Editar</button><button type="button" onClick={onDelete} className="pressable flex min-h-11 items-center justify-center gap-2 rounded-lg border border-destructive/25 px-3 text-xs font-semibold text-destructive hover:bg-destructive/5 xl:w-[130px]" data-testid={`button-delete-order-${order.id}`}><Trash2 size={16} /> Eliminar</button></>}
    </div>
  </article>;
}
